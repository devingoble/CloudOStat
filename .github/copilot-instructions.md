# Copilot instructions for CloudOStat

## Solution layout
- Shared UI lives in `CloudOStat.App/CloudOStat.App.Shared` (layouts, pages, theme, shared services). Prefer adding new UI and shared logic here.
- MAUI host is `CloudOStat.App/CloudOStat.App` (use `Platforms/` for platform-specific changes).
- Web server host is `CloudOStat.App/CloudOStat.App.Web`; WebAssembly client is `CloudOStat.App/CloudOStat.App.Web.Client`.
- Service defaults and common libraries are in `CloudOStat.ServiceDefaults` and `CloudOStat.Common`.

## Frameworks and UI
- Use .NET MAUI and Blazor; avoid Xamarin Forms.
- Prefer MudBlazor components for UI and theming.
- Keep responsive layout logic in `MainLayout.razor` and `MainLayout.razor.cs`.
- Place component-specific styles in `.razor.css` files; shared styles in `CloudOStat.App.Shared/wwwroot/app.css`.

## Conventions
- Target .NET 10 and C# 14; do not change TFMs or language version unless asked.
- Central package versions are managed in `Directory.Packages.props`; update versions there when adding packages.
- Follow existing namespace and folder structure; use file-scoped namespaces for new C# files.
- Keep public APIs minimal and add XML doc comments for new public members.

## Dependency injection and services
- Register shared services in `MauiProgram.cs`, `CloudOStat.App.Web/Program.cs`, and `CloudOStat.App.Web.Client/Program.cs` as needed.
- Use `NavigationService` from `CloudOStat.App.Shared/Services` for navigation definitions.

## Platform-specific guidance
- Windows-specific MAUI changes go in `CloudOStat.App/Platforms/Windows` or guarded by `#if WINDOWS`.
- Mobile safe-area adjustments should be handled in shared CSS.

## Testing and validation
- Prefer existing test frameworks in the solution and keep tests in separate `*.Tests` projects.
- Run a build after significant changes to ensure cross-target compatibility.
