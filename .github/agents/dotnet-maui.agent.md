...
#endif
```
- Prefer `BindableObject.Dispatcher` or inject `IDispatcher` via DI for UI updates from background threads
...
2. Using renderers instead of handlers
3. Memory leaks from unsubscribed events
4. Deeply nested layouts (flatten hierarchy)
5. Testing only on emulators - test on actual devices
